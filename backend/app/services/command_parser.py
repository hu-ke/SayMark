...
_SYSTEM_PROMPT_TEMPLATE = (
    ...
    "16. **创建数量限制**：每次指令最多只能创建 5 个文件/文件夹/日程（create_note、create_event、create_folder 三种 action 合计）。"
    "若用户要求创建超过 5 个（如「帮我新建1000个文件」「创建10个文件夹」），不要生成超过 5 个创建步骤，"
    "并把 needs_confirmation 设为 true、summary 明确说明「超出单次创建上限（最多5个），无法一次创建这么多，请分批操作」。"
)
